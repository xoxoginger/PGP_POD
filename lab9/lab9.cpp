    ...
    do 
    {
        if (bx < block.x - 1) 
        {
            for (k = 0; k < dim.z; k++)
            {
                ...
                { 
                    ...
                    edge_yz[k * dim.y + j] = data[_i(dim.x - 1, j, k)];
                }   
            }
            MPI_Send(edge_yz, dim.y * dim.z, MPI_DOUBLE, _ib(bx + 1, by, bz), id, MPI_COMM_WORLD);
        }

        if (bx > 0) 
        {
            MPI_Recv(edge_yz, dim.y * dim.z, MPI_DOUBLE, _ib(bx - 1, by, bz), _ib(bx - 1, by, bz), MPI_COMM_WORLD, &status);
            #pragma omp parallel for private(i, j, k) shared(data, edge_yz)
            for (k = 0; k < dim.z; k++)
            {
                for (j = 0; j < dim.y; j++)
                {
                    data[_i(-1, j, k)] = edge_yz[k * dim.y + j]; //buff[k * dim.y + j];
                }     
            }
        ...
        if (by < block.y - 1) 
        {
            for (k = 0; k < dim.z; k++)
            {
                for (i = 0; i < dim.x; i++)
                {
                    ...
                    edge_xz[k * dim.x + i] = data[_i(i, dim.y - 1, k)];
                }           
            }    
            MPI_Send(edge_xz, dim.z * dim.x, MPI_DOUBLE, _ib(bx, by + 1, bz), id, MPI_COMM_WORLD);
        }

        if (by > 0) 
        {
            MPI_Recv(edge_xz, dim.x * dim.z, MPI_DOUBLE, _ib(bx, by - 1, bz), _ib(bx, by - 1, bz), MPI_COMM_WORLD, &status);
            #pragma omp parallel for private(i, j, k) shared(data, edge_xz)
            for (k = 0; k < dim.z; k++)
            {
                for (i = 0; i < dim.x; i++)
                {
                    data[_i(i, -1, k)] = edge_xz[k * dim.x + i]; //buff[k * dim.x + i];
                }
        ...
        if (bz < block.z - 1) 
        {
            for (j = 0; j < dim.y; j++)
            {
                for (i = 0; i < dim.x; i++)
                {
                    edge_xy[j * dim.x + i] = data[_i(i, j, dim.z - 1)];
            ...
            //cout << r;
            MPI_Send(edge_xy, dim.y * dim.x, MPI_DOUBLE, _ib(bx, by, bz + 1), id, MPI_COMM_WORLD);
        }

        if (bz > 0) 
        {
            MPI_Recv(edge_xy, dim.x * dim.y, MPI_DOUBLE, _ib(bx, by, bz - 1), _ib(bx, by, bz - 1), MPI_COMM_WORLD, &status);
            #pragma omp parallel for private(i, j, k) shared(data, edge_xy)
            for (j = 0; j < dim.y; j++)
            {
                for (i = 0; i < dim.x; i++)
                {
                    data[_i(i, j, -1)] = edge_xy[j * dim.x + i]; //buff[j * dim.x + i];
                }    
            }
        ...
        if (bx > 0) 
        {
            ...
            MPI_Send(edge_yz, dim.z * dim.y, MPI_DOUBLE, _ib(bx - 1, by, bz), id, MPI_COMM_WORLD);
        }
        ...
        if (bx < block.x - 1) 
        {
            ...
            for (k = 0; k < dim.z; k++)
            {
                for (j = 0; j < dim.y; j++)
                {
                    data[_i(dim.x, j, k)] = edge_yz[k * dim.y + j]; //buff[k * dim.y + j];
                } 
            }    
        }
        else 
        ...
        if (bz < block.z - 1) 
        {
            ...
            for (j = 0; j < dim.y; j++)
            {
                for (i = 0; i < dim.x; i++)
                {
                    data[_i(i, j, dim.z)] = edge_xy[i + j * dim.x];
                }
            }
        }
        else 
        ...
        #pragma omp parallel for private(i, j, k) shared(data, next, hx, hy, hz)
        for (i = 0; i < dim.x; i++)
        {
            for (j = 0; j < dim.y; j++)
            {
